...
def get_newsupdate(account,latestpost):
    ...
    for attempt in range(3):
        try:
            ...
            for i, post in enumerate(sntwitter.TwitterUserScraper(account,isUserId = False).get_items()):
...
while True:
    ...
    accounts.columns = [1]
    if len(latest) < len(accounts):
        accounts[0] = [pd.to_datetime('1900-01-01')] * len(accounts)
        latest = pd.concat([latest,accounts[~(accounts[1].isin(latest[1]))]],axis=0)
    ...
    for index,row in latest.iterrows():
        for attempt in range(3):
            try:
                ...
                upload_db_latestnews(con,headlines,"headlines")
